...
def https_check(endpoint):
    ...
    try:
        ...
        # Ignore exception
        logging.exception("{}: Unknown exception examining certificate deployment.".format(endpoint.url))
            ...
            if cert_plugin_result.verified_certificate_chain is None:
                ...
                if(PT_INT_CA_FILE is not None):
                    try:
                        ...
                        command = ScanCommand.CERTIFICATE_INFO
                        command_extra_args = {
                            command: CertificateInfoExtraArguments(custom_ca_file=Path(PT_INT_CA_FILE))
                        }
                        scan_request = ServerScanRequest(
                            server_info=server_info,
                            scan_commands_extra_arguments=command_extra_args,
